...
-- Table `EasyInv`.`host`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `EasyInv`.`host` ( `idhost` INT NOT NULL, `regnum` INT NOT NULL, `hostname` VARCHAR(45) NOT NULL, `domainname` VARCHAR(45) NULL, `idcpu` INT NOT NULL, `idmemory` INT NOT NULL, `idHDD` INT NOT NULL, `idGPU` INT NOT NULL, `idhostsoft` INT NOT NULL, `screen` INT NULL, PRIMARY KEY (`idhost`), UNIQUE INDEX `hosttype_UNIQUE` (`idhost` ASC), INDEX `idHDD_idx` (`idHDD` ASC), INDEX `idCPU_idx` (`idcpu` ASC), INDEX `idmemory_idx` (`idmemory` ASC), INDEX `idGPU_idx` (`idGPU` ASC), UNIQUE INDEX `regnum_UNIQUE` (`regnum` ASC), INDEX `idhostsoft_idx` (`idhostsoft` ASC), CONSTRAINT `idHDD` FOREIGN KEY (`idHDD`) REFERENCES `EasyInv`.`hdd` (`idhdd`) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT `idCPU` FOREIGN KEY (`idcpu`)  REFERENCES `EasyInv`.`cpu` (`idcpu`) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT `idmemory`  FOREIGN KEY (`idmemory`)  REFERENCES `EasyInv`.`memory` (`idmemory`) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT `idGPU` FOREIGN KEY (`idGPU`) REFERENCES `EasyInv`.`gpu` (`idgpu`) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT `host_regnum` FOREIGN KEY (`regnum`) REFERENCES `EasyInv`.`devices` (`regnum`) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT `idhostsoft`  FOREIGN KEY (`idhostsoft`) REFERENCES `EasyInv`.`soft` (`idhostsoft`) ON DELETE CASCADE ON UPDATE CASCADE) ENGINE = InnoDB;
...
START TRANSACTION;
USE `EasyInv` ;
INSERT INTO admins (login, password, surname, name, otchestvo) VALUE ('administrator','12','BuiltIn','system', 'admin');
